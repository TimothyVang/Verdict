...
def detect_mode() -> Mode:
    ...
    local_available = has_local_inference_endpoint()
...
def has_local_inference_endpoint() -> bool:
    return bool(os.environ.get("SGLANG_BASE_URL"))


def mode_is_available(mode: Mode) -> bool:
    if mode is Mode.CLOUD:
        return has_cloud_credential()
    if mode is Mode.AIRGAP:
        return has_local_inference_endpoint()
