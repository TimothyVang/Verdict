...
**Document version 4.3 — May 2, 2026.** v4 added Langfuse + SqliteSaver + Plan-then-Execute. v4.1 patched the Hermes Agent characterization after source review. v4.2 fixed six self-review findings (cloud-only honesty, typed schemas, descope path, demo case engineering, fanout note, negative hypotheses). **v4.3 fixes ten system design review findings on cross-team interaction surfaces:** (1) `LedgerEntry` schema expanded to three explicit ID hierarchies (case / trace / checkpoint) — `trace_id` was ambiguous; (2) `PlanComprehensionEcho` model + new `comprehension_gate` LangGraph node added between fanout and execution to catch plan-comprehension drift before it produces false CONTESTED; (3) three-layer immutability defense made explicit (claude hook + langgraph wrapper + microsandbox mount) — air-gap mode otherwise loses the PreToolUse guarantee; (4) mode-at-case-init policy locked, `mode_at_case_init` field added to ledger, demo storyboard updated; (5) Haley's OpenLLMetry task expanded from +0.5 to +1 day with explicit token-count integration test; (6) Tim's TSI demo prep budgeted half-day with redaction pass on ledger payload; (7) Inspect AI sandbox now exposes `LANGFUSE_TRACE_ID` to scorers, KP ships three per-mode scorers (one per Mode); (8) fanout-checkpoint demo narration aligned with super-step semantics — kill-9 between super-steps, not mid-fanout; (9) architecture diagram caption updated for fanout granularity; (10) ground-truth sample count bumped from 30 to 50 across 3 cases to reduce variance.
...
These three schemas pin the contract between teammates' code. Tim, Beaver, Haley, KP code against them. Define before week 2.
...
class Hypothesis(BaseModel):
    ...
    mitre_technique: str | None               # e.g. "T1055"
...
class InvestigationPlan(BaseModel):
    ...
    # v4.3: per-executor comprehension echoes (replaces v4.2's single bool)
...
class LedgerEntry(BaseModel):
    ...
    event_type: Literal["case_init", "tool_call", "finding", "approval", "rejection", "mode_lock", "comprehension_check"]
...
**Tim (gateway / microsandbox / ledger / hooks / Inspect AI scaffolding)** — **+~4 days (was +3 in v4)**
...
- **(v4.3) Week 2 add:** LangGraph `executor_work` wrapper — Layer 2 of the three-layer immutability defense. Validates typed tool args against deny-rule list regardless of which model called. Fires in all three modes, including air-gap where Claude Code hooks don't.
...
**Beaver (LangGraph + verifier strategy + agent loop + prompt patterns)** — **+~4 days (was +3 in v4)**
...
**KP (forensics ground truth + tool wrappers + accuracy report)** — **+~1.5 days (was +1 in v4)**
