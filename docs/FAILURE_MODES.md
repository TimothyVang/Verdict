...
| Tool args | Invalid tool name, flag, PID, filter, or timeline option | Pydantic-AI `args_validator` before sandbox spawn | `ModelRetry`, max 2 | `tool_call` with validation error | After retry exhaustion, `Finding(status=UNVERIFIABLE, failure_reason="tool_args_failed_validation_after_2_retries")` |
