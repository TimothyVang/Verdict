...
**Status:** Phase 0 (mode-specific allowlists landed; credential-isolation hooks still deferred to W2). Engineering scaffolding, **not** part of the runtime authority chain. The Verdict runtime topology lives in `ARCHITECTURE.md`.
...
We therefore vendor **explicit mode-scoped allowlists** rather than relying on user-level config. The safe default `.mcp.json` is air-gap compatible; `.mcp.cloud.json` and `.mcp.dual.json` opt into network-capable developer MCPs. This gives us:
...
## 2. The six-MCP allowlist

| MCP | Origin | License | Transport | Purpose |
|---|---|---|---|---|
| `filesystem` | modelcontextprotocol/servers | **MIT** | stdio | Local workspace access for ledger / evidence manifest / case metadata under the working tree. **Excluded from safe default `.mcp.json`** because the upstream NPX server exposes write-capable tools. Available only in `.mcp.cloud.json` / `.mcp.dual.json` for development sessions until replaced with a read-only server or Docker read-only mount. |
| `fetch` | modelcontextprotocol/servers | **MIT** | stdio | Threat-intel enrichment, live MITRE ATT&CK lookup, SANS docs. **Excluded from safe default `.mcp.json` and `.mcp.airgap.json`**; available only in cloud/dual configs. |
...
| `context7` | upstash/context7-mcp | **MIT** | stdio | Up-to-date library/API docs for Pydantic v2, LangGraph, Inspect AI, Anthropic SDK, and other pinned implementation dependencies. Stateless; no credentials configured. |

Six — not ten. The Reddit consensus in 2026 is "the best Claude Code users run 2–3 plugins max"; we're above that, but only because each one maps to a concrete CLAUDE.md hard rule or a documented Verdict workflow. Anything more bloats prompt caches and dilutes the agent's attention budget.

**Current safety boundary:** `filesystem` and `fetch` are project-scoped developer MCPs, not runtime forensic controls, and are not loaded by the safe default config. Runtime evidence integrity still comes from `DenyRuleWrapper`, microsandbox read-only `/evidence`, hashing, and ledger validation. Air-gap sessions use `.mcp.json` or `.mcp.airgap.json`; cloud/dual development sessions may opt into `.mcp.cloud.json` or `.mcp.dual.json`.
...
### How mode-specific MCP configs reference secrets
...
```bash
# 1. No literal secrets in MCP configs
...
`.mcp.json` lives at the **repo root** and is committed as the safe default. It loads automatically when Claude Code starts in this directory. Mode-specific configs are also committed:

| Config | Intended use | Includes |
|---|---|---|
| `.mcp.json` | Safe default / air-gap compatible | `sequential-thinking` only |
| `.mcp.airgap.json` | Explicit air-gap launch | `sequential-thinking` only |
| `.mcp.cloud.json` | Cloud development/research | all six allowlisted MCPs |
| `.mcp.dual.json` | Dual-mode lab development | all six allowlisted MCPs |

```bash
# 1. Set required env vars (in shell or .env)
export GITHUB_TOKEN="ghp_..."   # required for the github MCP

# 2. Verify safe default
claude mcp list   # should show: sequential-thinking

# 3. For cloud/dual research sessions, launch Claude Code with the matching
#    explicit MCP config. Do not use these configs for air-gap cases.
claude --mcp-config .mcp.cloud.json
claude --mcp-config .mcp.dual.json

# 4. Adding a new MCP — gate
...
#    d. Update the relevant .mcp*.json file + this doc + .env.example
```
...
- **Hook-level credential and mode check.** A `PreToolUse` hook in `.claude/settings.json` that runs the §4 checklist before every MCP call and blocks `fetch` / `filesystem` when `VERDICT_MODE=airgap`. Tracked under `[W2.D.4]` (observability + ops).
...
- **Don't pile on.** Six MCPs is the upper end. New entries require a clear hard-rule or documented-workflow justification, not "this looks useful".
...
- [upstash/context7](https://github.com/upstash/context7) — Context7 MCP, MIT
