...
3. **Anthropic API key** or Claude Code OAuth for cloud-mode and dual-mode runs. `OPENROUTER_API_KEY` is an optional host-side fallback for runtime inference. Air-gap mode does not require cloud credentials. OAuth/API tokens are *not* redistributable — each contributor uses their own, and secrets never enter microVMs.
