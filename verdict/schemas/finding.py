...
#
# Two of the seven Tier-1 caveats are NOT pure class-membership triggers
# and are deliberately not encoded here:
#
#   * LOGON_TYPE_3_VS_10 — keyed by EVTX_4624 AND
...
#     wrapper at parse time. (Also: EVTX_4624 is not yet in the
#     ArtifactClass enum — see W1.B.1 for the canonical 13-member set.)
#
#   * SYSMON_PROCESSGUID_OVER_PID — triggered by a *correlation step*
#     that uses PID alone, not by SYSMON_1 citation per se. A Finding
#     can legitimately cite SYSMON_1 while keying its correlation on
#     ProcessGuid; that is the desired behaviour, and the caveat would
#     fire on a wrong-shaped *correlation*, not a citation. Enforcement
#     belongs in the correlator, not the Finding schema.
#
