...
thereafter (CLAUDE.md §3.4). The schema enforces this via ConfigDict(frozen=True).
...
Chain integrity: prev_entry_hash + hmac_sig form the HMAC-signed append-only chain.
Credential isolation (CLAUDE.md §3.9): auth fields are redacted before hashing;
payload_redactions records which keys were stripped.
"""
...
class LedgerEntry(BaseModel):
    ...
    finding_id: str | None
    """None for case_init / infrastructure events; populated for finding/approval/rejection."""
    ...
    """UTC timestamp with tzinfo; must have trailing Z when serialised."""
    ...
    # ------------------------------------------------------------------
    # Langfuse cross-references — explicit hierarchy (ARCHITECTURE.md §5)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # LangGraph cross-references — explicit hierarchy (ARCHITECTURE.md §5)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    payload: dict
    """Event-type-specific structured payload (not redacted version)."""
    ...
    per CLAUDE.md §3.9. This field records what was removed for auditability."""

    prev_entry_hash: str
    """blake3 digest of the previous entry — forms the append-only chain."""

    hmac_sig: str
    """HMAC-SHA256 over (payload after redaction + prev_entry_hash + entry_id).
